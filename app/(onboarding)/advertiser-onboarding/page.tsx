...
import { PersonalInfoStep } from "../_components/advertiser-steps/personal-info-step";
import { CompanyInfoStep } from "../_components/advertiser-steps/company-info-step";
import { IndustrySelectionStep } from "../_components/advertiser-steps/industry-selection-step";
import { LeadPreferencesStep } from "../_components/advertiser-steps/lead-preferences-step";
import { CampaignDetailsStep } from "../_components/advertiser-steps/campaign-details-step";
import { FORM_STEPS } from "@/constants/advertiser";
import { AdvertiserFormData } from "@/types/advertiser";

const INITIAL_DATA: AdvertiserFormData = {
  firstName: "",
  lastName: "",
  phoneNumber: "",
  jobTitle: "",
...
export default function AdvertiserOnboardingPage() {
  ...
  const [formData, setFormData] = useState<AdvertiserFormData>(INITIAL_DATA);

  function updateFields(fields: Partial<AdvertiserFormData>) {
  ...
  const steps = [
    ...
    <LeadPreferencesStep key="leads" {...{ data: formData, updateFields }} />,
    <CampaignDetailsStep key="campaign" {...{ data: formData, updateFields }} />,
  ];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await saveOnboarding({
        ...
        companyWebsite: formData.hasCompany ? formData.companyWebsite : undefined,
        companySize: formData.hasCompany ? formData.companySize : undefined,
        industries: formData.targetIndustries,
        subIndustries: formData.targetSubIndustries,
        leadPreferences: formData.leadPreferences,
